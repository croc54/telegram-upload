...
class TelegramUploadClient(TelegramClient):
    ...
    def send_one_file(
        ...
        retries=RETRIES,
        reporter: Optional[TelegramProgressReporter] = None):
        ...
        if owns_reporter:
            ...
            self.loop.run_until_complete(reporter.start())
        ...
        try:
            try:
                ...
                if send_as_media:
                    message = async_to_sync(
                        self._send_media(entity, file, progress_callback)
                    )
                else:
                    message = self._send_file_message(
                        entity, file, thumb, progress_callback
                    )
            finally:
                bar.render_finish()
            ...
            if retries > 0:
                click.echo(
                    f'The file "{file.file_name}" could not be uploaded: {e}. Retrying...',
                    err=True,
                )
                message = self.send_one_file(
                    entity,
                ...
                click.echo(
                    f'The file "{file.file_name}" could not be uploaded: {e}. '
                    f'It will not be retried.',
                    err=True,
                )
                self.loop.run_until_complete(reporter.fail(e))
                return None

        if owns_reporter and message is not None:
            self.loop.run_until_complete(reporter.finish())
